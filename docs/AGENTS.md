...
**تأیید روی دستگاه واقعی (commit `9668812`):** ۱۸ تست `bana-env-scanner`
سبز (۱۰ میزبان + ۳ JDK + ۵ SDK). `bana doctor` SDK واقعی را در
`/usr/lib/android-sdk` با `platforms: android-35, android-34` و
`build-tools: debian, 34.0.0` گزارش داد.

**نکته‌ی واقعی برای فاز build_driver (نه باگ):** زیر `build-tools/` یک
پوشه‌ی غیراستاندارد به اسم `debian` هم هست (احتمالاً از پکیج‌بندی apt کالی)
که شماره‌نسخه نیست. تشخیص فعلی درست عمل کرد (واقعیت دیسک را گزارش داد، نه
حدس)، ولی هر منطق آینده که «آخرین/بهترین build-tools» را برای build واقعی
انتخاب می‌کند باید فقط پوشه‌های شبیه شماره‌نسخه (مثل `34.0.0`) را کاندید
حساب کند، نه هر نامی زیر این مسیر.
